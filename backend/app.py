...
# --- Improved path manipulation for clarity and robustness ---
# Use .parents[1] for explicit "two levels up" and check before appending.
...
# --- Centralized logging format for easier maintenance ---
...
def create_app(config: Config) -> Flask:
    ...
    def before_request_func():
        ...
        to access these instances via `g.downloader` or `g.tasks` without
        global variables or repeated instantiation.
        """
...
def setup_logging(config: Config):
    ...
    logging.basicConfig(
        level=log_level,
        format=LOG_FORMAT,  # Use the defined constant
        handlers=[
...
def main():
    ...
    # Print our custom, reliable signal to stdout for the Electron process.
    # The `flush=True` is critical to ensure the output is not buffered.
