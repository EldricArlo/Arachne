...
def create_download_task(
    url: str, user_options: Dict[str, Any], downloader: "YouTubeDownloader"
) -> str:
    """
    ...
    with _tasks_lock:
        if task_id in _tasks:
            ...
            if new_status and new_status != current_status:
                logger.debug(
                    f"[Task {task_id}] Status update: {current_status} -> {new_status}"
                )

            _tasks[task_id].update(progress_data)
        else:
            logger.warning(
                f"[Task {task_id}] Attempted to update a task that no longer exists."
            )
...
def _download_worker(
    ...
    try:
        ...
        logger.error(
            f"[Task {task_id}] Uncaught exception in worker thread: {e}", exc_info=True
        )
        _update_task_progress(
            task_id,
            {
                "status": "error",
                "error": "A critical error occurred in the worker thread.",
            },
        )
