# backend/config.py
"""
Manages the application's configuration via a YAML file.

This module defines the default settings, loads user configurations from
`config.yaml`, handles creation of the default config if it's missing,
...
# These are the application's "factory settings." If config.yaml is missing
# or corrupted, the program will fall back to these values.
DEFAULT_CONFIG = {
    # --- Core Paths ---
    "downloads_dir": None,
...
class Config:
    ...
    def __init__(self):
        ...
        It immediately attempts to load config.yaml; if that fails, a new
    ...
    def _update_paths(self):
        """
        Dynamically sets and creates key directory paths based on the current config.
        This method ensures that path-dependent properties are always in sync.
        """
        # --- Set and resolve download directory ---
        if not self.downloads_dir or not str(self.downloads_dir).strip():
            default_path = self.BASE_DIR / "downloads"
            self.DOWNLOADS_DIR = default_path
            self.downloads_dir = str(default_path)  # Ensure string representation is updated
        else:
            user_path = Path(self.downloads_dir).expanduser().resolve()
            self.DOWNLOADS_DIR = user_path
            self.downloads_dir = str(user_path) # Ensure string is the resolved path
        ...
        # --- Set yt-dlp output template based on the current download path ---
        self.YTDLP_DEFAULT_OUTTMPL = str(self.DOWNLOADS_DIR / "%(title)s.%(ext)s")

    def _load_or_create_config(self):
        """
        Load config.yaml or create it with defaults if it doesn't exist.

        If the file exists but is corrupted, the default configuration is used
        ...
        # Populate self with loaded or default values
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        This is useful for API responses or for saving to the YAML file.
    ...
    def update(self, new_config: Dict[str, Any]):
        """
        Update configuration attributes from a dictionary and save to file.

        Args:
            new_config (Dict[str, Any]): A dictionary of new settings.
        """
        ...
        # If the download directory changed, regenerate all dependent paths
        if path_changed:
            self._update_paths()
    ...
    def save(self):
        """Persist the current state of all configuration items to config.yaml."""
...
def get_config() -> Config:
    """
    Get the global singleton configuration instance in a thread-safe way.

    This function ensures that there is only one `Config` object throughout
