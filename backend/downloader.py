...
from backend.ytdlp_utils import build_download_options
...
class YouTubeDownloader:
    ...
    def _progress_hook(self, d: Dict[str, Any]):
        ...
        if status == 'downloading':
            ...
            logger.info(f"File download finished: {Path(filename).name}. Waiting for post-processing...")
            # Status 'finished' means the raw file is downloaded; ffmpeg may still merge or convert.
            self.progress_callback({'status': 'processing', 'message': 'Merging formats...'})
        elif status == 'error':
            logger.error("yt-dlp progress hook reported a download error.")
            self.progress_callback({'status': 'error', 'error': 'Unknown error during download'})
    ...
    def get_video_info(self, url: str) -> Dict[str, Any]:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                logger.info(f"Successfully fetched video info: '{info.get('title', 'N/A')}'")
    ...
    def delete_file(self, filename: str):
        ...
        if not str(file_path).startswith(str(base_dir)):
            logger.warning(f"Refusing to delete file outside downloads directory: {file_path}")
            raise PermissionError("Deletion of files outside the downloads directory is not allowed.")
        ...
        try:
            ...
            logger.error(f"Unknown error while deleting file '{file_path}': {e}", exc_info=True)
            raise e