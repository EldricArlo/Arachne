...
class YouTubeDownloader:
    ...
    def _progress_hook(self, d: Dict[str, Any]):
        ...
        if status == "downloading":
            ...
            # --- MODIFICATION: Provide specific error message from yt-dlp ---
            error_message = d.get('errmsg', "Unknown error during download")
            logger.error(f"yt-dlp progress hook reported an error: {error_message}")
            self.progress_callback(
    ...
    def download(self, url: str, user_options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # --- MODIFICATION: Simplified pre-download check for FFmpeg ---
        # FFmpeg is required for merging streams or converting formats for nearly all downloads.
        ffmpeg_path = find_ffmpeg()
        if not ffmpeg_path:
            error_message = "FFmpeg not found. It is required for downloading. Please install FFmpeg and ensure it's in the system's PATH."
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # The download method can raise exceptions, especially during post-processing
                ydl.download([url])
            return {"success": True}
        ...
        for file_path in self.config.DOWNLOADS_DIR.iterdir():
            if file_path.is_file() and not file_path.name.startswith("."):
                try:
                    ...
                    # File may have been deleted during iteration; ignore safely.
                    continue
    ...
    def delete_file(self, filename: str):
        ...
        # Security: re-validate that the resolved path is inside the downloads directory
