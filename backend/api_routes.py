...
from yt_dlp.utils import (
    DownloadError,
)  # Import yt-dlp's specific exception for more precise error handling
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "An internal error occurred while fetching the configuration.",
                }
            ),
            500,
        )
...
def update_app_config():
    ...
    try:
        ...
        if not new_config_data:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "No configuration data provided in the request body.",
                    }
                ),
                400,
            )
        ...
        logger.info(
            "Application configuration has been successfully updated and saved."
        )
        return jsonify(
            {
                "success": True,
                "message": "Configuration has been successfully updated and saved.",
            }
        )
    except Exception as e:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "An internal error occurred while updating the configuration.",
                }
            ),
            500,
        )
...
def get_video_info():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": f"Could not process this URL: {error_msg}"}
            ),
            400,
        )
    except Exception as e:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "An internal error occurred while fetching video info, please check logs.",
                }
            ),
            500,
        )
...
def start_download():
    ...
    try:
        ...
        if g.tasks.get_active_downloads_count() >= config.max_concurrent_downloads:
            logger.warning(
                "Download request rejected: concurrent download limit reached."
            )
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Concurrent download limit reached, please try again later.",
                    }
                ),
                429,
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "An internal error occurred while starting the download task, please check logs.",
                }
            ),
            500,
        )
...
def get_download_progress(task_id: str):
    ...
    if not status:
        return (
            jsonify(
                {"status": "not_found", "error": "Task does not exist or has expired"}
            ),
            404,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "An internal error occurred while deleting the file, please check logs.",
                }
            ),
            500,
        )
