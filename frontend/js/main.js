...
class App {
    ...
    async handleDownload(options) {
        ...
        try {
            ...
            // Add the new task UI element to the single video page for immediate feedback.
            ui.addTaskToSingleView(taskData);
            
            this._startProgressPolling();

            // Clear the video info card, but keep the URL input for context.
            this.currentVideoInfo = null;
            ui.clearVideoInfo();
        } catch (error) {
    ...
    async handleStartBatchDownload() {
        ...
        try {
            const { message, created_tasks } = await api.startBatchDownload(urls);
    ...
    _startProgressPolling() {
        ...
        this.progressInterval = setInterval(async () => {
            if (this.activeTasks.size === 0) {
                ...
                // Once polling stops, refresh the history to show the final files.
                this.handleRefreshHistory();
                // And clear the temporary task list from the single view.
            ...
            for (const taskId of [...this.activeTasks.keys()]) {
                try {
                    ...
                    // This check prevents a rare race condition where a task is removed
                    // between the start of the loop and this point.
                    if (!existingTask) continue; 

                    // Merge new progress into the existing state
                    const updatedTask = { ...existingTask, ...progressStatus, id: taskId };
                    
                    // Update central state
                    this.activeTasks.set(taskId, updatedTask);

                    // Command the UI to update its representation of the task
                    ui.addOrUpdateTask(updatedTask);

                    // If the task has reached a terminal state, remove it from active polling for the next cycle
                    if (['completed', 'error', 'not_found'].includes(progressStatus.status)) {
                        this.activeTasks.delete(taskId);
                    }
                } catch (error) {
                    console.error(`Error polling task ${taskId}:`, error);
                    this.activeTasks.delete(taskId); // Stop polling a failing task
                }
            }
        }, 1000);
    }
