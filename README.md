...
第二个版本了，虽然还是不能成功的下载视频，但是也算是一个伟大的进步了
1. 这个版本优化了 findPythonExecutable 函数，使其在 macOS 和 Linux 系统上会优先尝试使用 python3 命令，如果找不到再回退到 python。这提高了在不同操作系统上找到正确 Python 解释器的成功率;
2. 此版本改进了所有 API 路由的错误处理。当服务器发生意外错误时，它现在会向日志文件记录详细的堆栈信息（便于开发者调试），同时只向前端返回一个通用的、对用户友好的错误提示，避免了内部实现细节的泄露;
