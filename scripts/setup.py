...
def run_command(command: str, description: str, cwd: Path = PROJECT_ROOT):
    """
    Runs a shell command and provides clear log output.
    ...
    try:
        ...
        print(f"❌ Command '{cmd_name}' not found. Please ensure it is installed and in the system's PATH environment variable.")
        return False
...
def check_python_version():
    ...
    if version.major < 3 or (version.major == 3 and version.minor < 8):
        print(f"❌ Python 3.8 or higher is required, current version: {platform.python_version()}")
        return False
...
def check_node_version():
    ...
    try:
        ...
        print("❌ Node.js is not installed or not in PATH. Please visit https://nodejs.org/ to download and install the LTS version.")
        return False
...
def install_python_dependencies():
    ...
    if not requirements_file.exists():
        print(f"❌ Dependency file {requirements_file} does not exist, cannot install Python dependencies.")
        return False
    
    pip_executable = f'"{sys.executable}" -m pip'
    run_command(f'{pip_executable} install --upgrade pip', "Upgrading pip")
    return run_command(f'{pip_executable} install -r "{requirements_file}"', "Installing Python dependencies")

# --- Core Modification: Implement smart check for Node.js dependencies ---
def install_node_dependencies():
    """
    Checks if Node.js dependencies exist and are up-to-date.
    If the 'node_modules' directory does not exist, or if the 'package.json' file is newer,
    it executes 'npm install'. Otherwise, it skips the installation.
    """
    package_json_path = PROJECT_ROOT / "package.json"
    node_modules_path = PROJECT_ROOT / "node_modules"

    if not package_json_path.exists():
        print(f"❌ {package_json_path} file does not exist, cannot install Node.js dependencies.")
        return False
    ...
    if node_modules_path.is_dir():
        try:
            ...
            if package_mod_time < node_modules_mod_time:
                ...
                print(f"✅ 'node_modules' directory already exists and is up-to-date, skipping installation.")
                print("="*50)
                return True
        except FileNotFoundError:
    ...
    return run_command('npm install', "Installing/Updating Node.js dependencies", cwd=PROJECT_ROOT)
...
def check_ffmpeg():
    ...
    try:
        ...
        print("⚠️  FFmpeg not found. It is a core component for merging audio/video and format conversion.")
        return False

def install_ffmpeg_instructions():
    """Displays installation instructions for ffmpeg based on the operating system."""
    ...
    if system == 'darwin':  # macOS
        ...
        print("2. After unzipping, add the full path to the 'bin' directory to your system's 'Path' environment variable.")
    
    print("="*50 + "\n")

def create_directories():
    """Creates the directories required for the application to run."""
...
def main():
    """Main function to execute all setup steps in order."""
    ...
    all_ok = (
        ...
        install_node_dependencies() # <-- Now calls our modified smart function
    )
...
if __name__ == '__main__':
    if not (PROJECT_ROOT / "frontend").is_dir() or not (PROJECT_ROOT / "backend").is_dir():
        print(f"❌ Error: This script does not seem to be in the expected project structure.")
        print(f"   Please ensure that the 'frontend' and 'backend' directories are at the same level as the 'scripts' directory.")
